...
Spring 2021
Project 1
OpenMP: Monte Carlo Simulation (cannonball hits the castle)
************************************************************/
...
const float VMIN = 10.0;				//intial cannonball velocity in meters/sec
const float VMAX = 30.0;				//intial cannonball velocity in meters/sec
			...
			if (x <= g) {
				...
				float y = vy * dt + 0.5 * (-9.8) * dt * dt;
					if (y <= h) {
						...
						disc = sqrtf(disc);
						float dt1 = (-b + disc) / (2.f * a);		//time to intersect high ground
						float dt2 = (-b - disc) / (2.f * a);		//time to intersect high ground
						float dtmax = dt1 > dt2 ? dt1 : dt2;		//only care about the second intersection

						//how far does the ball land horizontally from the edge of the cliff?
						float upperDist = vx * dtmax - g;

						//see if the ball hits the castle
						if (fabs(d - upperDist) > TOL) {
							if (DEBUG) fprintf(stderr, "Misses the castle at upperDist = %8.3lf\n", upperDist);
						}
						else {
...
//generate a random number between two ints
...
//a different random number sequence everytime you run it:
void TimeOfDaySeed() {
	struct tm y2k = { 0 };
	y2k.tm_hour  = 0;
	y2k.tm_min   = 0;
	...
	double seconds = difftime(timer, mktime(&y2k));				//the number of seconds since 00:00 hours, Jan 1, 1970 UTC (i.e., the current unix timestamp)
