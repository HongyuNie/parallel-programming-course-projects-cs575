...
Spring 2021
Project 5
Monte Carlo Simulation (cannonball hits the castle)
*******************************************/
...
MonteCarlo( float *dvs, float *dths, float *dgs, float *dhs, float *dds, int *dhits )
{
        ...
        float x = vx * t;
        // the ball reach the cliff face:
        if( x > g )
        {
            // what time the ball gets even with x = g:
            t =  g / vx;
            // how far the ball went vertically in that time:
            float y = vy * t + 0.5 * GRAVITY * t * t;
                // the ball reach the upperdeck:
                if (y > h)
                {
                    float a = 0.5 * GRAVITY;
                    float b = vy;
...
main( int argc, char* argv[ ] )
{
        ...
        int   *hhits = new int   [NUMTRIALS];  //put all zero
    ...
    for (int i = 0; i < NUMTRIALS; i++)
    {
        ...
        printf("The elasped time  = %6.2lf\n", msecTotal);
        double megaTrialPersecond = (double)NUMTRIALS / msecTotal /1000000;
        printf("The Performance  = %6.2lf\n", megaTrialPersecond);
        ...
        printf("The Probablity is %lf\n", (float)sum / (float)NUMTRIALS);
