...
Spring 2022
Project 6
Part 1 and Part 2: OpenCL Array Multiply + Multiply-Add
********************************************************/
...
6. Write the data from the host buffers to the device buffers7. Read the kernel code from a file
...
#ifndef NMB					// number of mega bytes
#define	NMB					64
#endif
...
main( int argc, char *argv[ ] )
{
	...
	// float hA[ NUM_ELEMENTS ];  -- Global memory and the heap typically have lots more space than the stack does. 
	//                               So, typically, you do not want to allocate a large array like this as a local variable.
	float *hA = new float[ NUM_ELEMENTS ]; //Here, it’s being done on the heap. It could also have been done in global memory
	...
	cl_context context = clCreateContext( NULL, 1, &device, NULL, NULL, &status ); //paras:properties, one device, the device, Callback, pass in user data, return status.
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateContext failed\n" );

	// 4. create an opencl command queue:

	cl_command_queue cmdQueue = clCreateCommandQueue( context, device, 0, &status ); // - is properties
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateCommandQueue failed\n" );

	// 5. allocate the device memory buffers:

	cl_mem dA = clCreateBuffer( context, CL_MEM_READ_ONLY,  dataSize, NULL, &status ); //CL_MEM_READ_ONLY means that the OpenCL device can only get this data – it can’t send it back to the host CPU.
	if( status != CL_SUCCESS )
	...
	// 9. create the kernel object:
	cl_kernel kernel = clCreateKernel( program, KERN_FUNC, &status );
	if( status != CL_SUCCESS )
	...
	fprintf( stderr, "%4d\t%4d\t%10.3lf MegaMultsPerSecond\n",
