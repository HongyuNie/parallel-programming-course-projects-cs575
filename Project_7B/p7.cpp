...
Spring 2022
Project 7B
Autocorrelation using MPI (Message Passing Interface)
********************************************************/
...
main( int argc, char *argv[ ] )
{
	...
	MPI_Init( &argc, &argv );	// MPI_Init( NULL, NULL ) if don’t need to process command line arguments

	// Getting information about our place in the communicator:
	// Communicator stands for a collection of CPUs that are capable of sending messages to each other
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			if( dst == BOSS )
				continue;
			//TODO:
			MPI_Send( &BigSignal[dst*PPSize], PPSize+MAXSHIFTS, MPI_FLOAT, dst, TAG_SCATTER, MPI_COMM_WORLD ); //on mpi notes page 20
		}
	}
	else
	{
		//TODO:
		MPI_Recv( PPSignal, PPSize+MAXSHIFTS, MPI_FLOAT, BOSS, TAG_SCATTER, MPI_COMM_WORLD, &status ); //on mpi notes page 21
	}
